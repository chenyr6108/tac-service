...
public class AvgPeriodReportCommand extends BaseCommand {
	...
	public void getReport(Context context) throws Exception{
		...
		chartInfo.setAlertTitle("{dataSetName}{xLabel}的平均租期是{value}");
		//设置X轴
		chartInfo.setxLabel(new String[]{"1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"});
		chartInfo.setChartName("avgPeriod");
		chartInfo.setChartTitle("平均租期走势图");
		chartInfo.setyTitle("平均租期");
		...
		Output.jspOutput(outputMap, context, "/report/avgPeriod_Report.jsp");
	}
	...
	public void getReportForTable(Context context) throws Exception{
		...
		for (SelectionTo o : baseService.getAllOffice()) {
			...
			data = getLineByOneForTable(year, o.getDisplay_name(), busi_type, code_type);
			dataSets.add(data);
		}
		...
		Output.jspOutput(outputMap, context, "/report/avgPeriod_table.jsp");
	}
	...
	public ChartDataSet getLineByOne(String year, String decp, String busi_type, String code_type){
		...
		for (int i = 1; i <= yData.length; i++) {
			...
			yData[i - 1] = (Double) baseService.queryForObj("report.getAvgPeriodReportByYearAndMonth", paramMap);
		}
	...
	public ChartDataSet getLineByOneForTable(String year, String decp, String busi_type, String code_type){
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("decp", decp);
		paramMap.put("busi_type", busi_type);
		paramMap.put("code_type", code_type);
		...
		data.setTitle(StringUtils.isEmpty(decp) ? "合计" : decp);
		data.setyData(yData);
		return data;
	} 
