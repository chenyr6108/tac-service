 ...
 * 导出客户虚拟账号
 */
public class ExportCustomerVirtualCodePDF extends AService {
	Log logger = LogFactory.getLog(ExportCustomerVirtualCodePDF.class);

	@SuppressWarnings("unchecked")
	public void getCustomerVirtualCodeData(Context context) {

		List customerVirtualCode = new ArrayList();
		Map customer =null;
		try {
			DataAccessor.getSession().startTransaction();
			customerVirtualCode = (List) DataAccessor
					.query("customer.getCustomerVirtualCode",
							context.getContextMap(),
							DataAccessor.RS_TYPE.LIST);
			
			this.expCustomerVirtualCodeData(context, customerVirtualCode);
			
			for (int i = 0; i < customerVirtualCode.size(); i++) {
				customer = (Map) customerVirtualCode.get(i);
				DataAccessor.getSession().update("customer.updateCustomerExportFlag", customer);
			}
