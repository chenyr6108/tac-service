...
import com.brick.credit.to.SupportTO;
import com.brick.credit.to.SupportTO.KeyOfMap;
...
public class CreditPaylistService extends AService {
	...
	public void showCreditPaylist (Context context) throws SQLException {
				...
				if (schema != null) {
					...
					if("5".equals(schema.get("TAX_PLAN_CODE"))) {
						...
						if (schema != null) {
							...
							outputMap.put("incomePayTax", schema.get("INCOME_PAY_TAX"));
							outputMap.put("incomePayType", schema.get("INCOME_PAY_TYPE"));
		...
		if(schema!=null) {
			List<Map<String,Object>> payLines=(List<Map<String,Object>>)(paylist.get("paylines"));
			if((schema.get("PAY_WAY")+"").equals("21")||(schema.get("PAY_WAY")+"").equals("23")) {//期末的需要调整支付日
				for(int i=0;payLines!=null&&i<payLines.size();i++) {
					Calendar cal=Calendar.getInstance();
					cal.setTime((Date)payLines.get(i).get("PAY_DATE"));
					cal.add(Calendar.MONTH,1);
					payLines.get(i).put("PAY_DATE",cal.getTime());
				}
			}
		}
		if (errList.isEmpty()) {
...
//			}
	...
	@SuppressWarnings("unchecked")
	public void createCreditPaylist (Context context) throws SQLException {
		...
		if(errList.isEmpty()){	
			...
			try {				
				...
				//
				if(schema!=null&&(Constants.TAX_PLAN_CODE_5.equals(schema.get("TAX_PLAN_CODE"))||Constants.TAX_PLAN_CODE_7.equals(schema.get("TAX_PLAN_CODE")))) {
					paylist.put("SALES_PAY", schema.get("SALES_PAY"));
				}
				outputMap.put("paylist", paylist);
...
//			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void calculateCreditPaylist (Context context) throws SQLException {
...
//			}
	...
	public static Map copySchema(Map dest, Map src) throws ParseException {
		...
		dest.put("SALES_PAY", src.get("pay__money"));
		double m1=StringUtils.isEmpty((String)src.get("MANAGE_FEE"))==true?0:Double.valueOf(src.get("MANAGE_FEE").toString());
		double m2=StringUtils.isEmpty((String)src.get("MANAGE_FEE2"))==true?0:Double.valueOf(src.get("MANAGE_FEE2").toString());
		dest.put("MAGR_FEE",Math.round((m1+m2)/1.06));//管理费
		return dest;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void calculateCreditPaylistIRR (Context context) throws SQLException {
			...
			try {
				...
				try {
					...
					for (int i = 0; i < feeSetLists.size(); i++) {
						...
						}else if(Constants.TAX_PLAN_CODE_1.equals(schema.get("TAX_PLAN_CODE"))||
								Constants.TAX_PLAN_CODE_3.equals(schema.get("TAX_PLAN_CODE"))||
								Constants.TAX_PLAN_CODE_4.equals(schema.get("TAX_PLAN_CODE"))||
								Constants.TAX_PLAN_CODE_5.equals(schema.get("TAX_PLAN_CODE"))||
								Constants.TAX_PLAN_CODE_6.equals(schema.get("TAX_PLAN_CODE"))||
								Constants.TAX_PLAN_CODE_7.equals(schema.get("TAX_PLAN_CODE"))||
								Constants.TAX_PLAN_CODE_8.equals(schema.get("TAX_PLAN_CODE"))){
				...
				outputMap.put("incomePayTax", context.contextMap.get("incomePayTax"));
				outputMap.put("incomePayType", context.contextMap.get("incomePayType"));
				...
				paylist.put("PLEDGE_ENTER_MCTOAGRATE", context.contextMap.get("PAY_PLEDGE_ENTER_MCTOAGRATE"));
				if(schema!=null&&(Constants.TAX_PLAN_CODE_5.equals(schema.get("TAX_PLAN_CODE"))||Constants.TAX_PLAN_CODE_7.equals(schema.get("TAX_PLAN_CODE")))) {
					paylist.put("SALES_PAY", context.contextMap.get("pay__money"));
		...
		if(schema!=null) {
			...
			//如果支付表某期金额有负数需要卡住不得保存
			for(int i=0;payLines!=null&&i<payLines.size();i++) {
				if(Double.valueOf(payLines.get(i).get("OWN_PRICE").toString())<0||Double.valueOf(payLines.get(i).get("REN_PRICE").toString())<0) {
					outputMap.put("LOCK_SAVE","Y");
					break;
				}
			}
		}
		...
		//来自业务支撑的操作
		if(StringUtils.isEmpty(context.contextMap.get("functionFrom"))||"N".equals(context.contextMap.get("functionFrom")+"")) {
			outputMap.put("SUPPORT","N");
		} else {
			outputMap.put("SUPPORT",context.contextMap.get("functionFrom"));//业务支撑跳转过来的
			if(paylist!=null) {
				//业务支撑更新支付表处理
				SupportTO spTO=new SupportTO();
				spTO.setCreditId(context.contextMap.get("credit_id")+"");//报告号
				spTO.setLeaseTotalVal(context.contextMap.get("PAY_LEASE_TOPRIC")+"");//设备总价款
				spTO.setLeaseRZE(context.contextMap.get("PAY_LEASE_RZE")+"");//概算成本
				spTO.setLeasePeriod(context.contextMap.get("PAY_LEASE_PERIOD")+"");//租赁期数
				spTO.setPayWay(context.contextMap.get("PAY_PAY_WAY")+"");//支付方式
				spTO.setDeposit(context.contextMap.get("PAY_PLEDGE_PRICE")+"");//保证金总额
				spTO.setTaxPlanCode(context.contextMap.get("PAY_TAX_PLAN_CODE")+"");//税费方案
				spTO.setPayMoney(context.contextMap.get("pay__money")+"");//佣金
				spTO.setIncomePayType(context.contextMap.get("incomePayType")+"");//手续费收入类型
				spTO.setIncomePay(context.contextMap.get("incomePay")+"");//手续费收入
				spTO.setOutPay(context.contextMap.get("outPay")+"");//银行手续费
				spTO.setFirstIrrMonthPrice(context.contextMap.get("PAY_HEAD_HIRE")+"");//首期租金
				spTO.setYearInterest(context.contextMap.get("PAY_YEAR_INTEREST")+"");//合同利率
				spTO.setDepositA(context.contextMap.get("PAY_PLEDGE_AVE_PRICE")+"");//平均抵充或者一次抵充
				spTO.setDelayPay(Integer.valueOf(context.contextMap.get("PAY_DEFER_PERIOD")+""));//延迟拨款期数
				spTO.setDepositBNum(Integer.valueOf(context.contextMap.get("PAY_PLEDGE_LAST_PERIOD")+""));//用于抵充最后期数
				spTO.setDepositB(context.contextMap.get("PAY_PLEDGE_LAST_PRICE")+"");//用于抵充最后期数保证金
				spTO.setDepositC(context.contextMap.get("PAY_PLEDGE_BACK_PRICE")+"");//用于期末返还
				spTO.setToCompany(context.contextMap.get("PAY_PLEDGE_ENTER_CMPRICE")+"");//入我司
				spTO.setToSupplier(context.contextMap.get("PAY_PLEDGE_ENTER_AG")+"");//入供应商
				spTO.setCompanyToSupplier(context.contextMap.get("PAY_PLEDGE_ENTER_MCTOAG")+"");//我司入供应商
				spTO.setManageFee1(context.contextMap.get("MANAGE_FEE")+"");//管理费1
				spTO.setManageFee2(context.contextMap.get("MANAGE_FEE2")+"");//管理费2
				spTO.setHomeFee(context.contextMap.get("home_fee")+"");//家访费
				spTO.setSetupFee(context.contextMap.get("setup_fee")+"");//设定费
				spTO.setOtherFee(context.contextMap.get("other_fee")+"");//其他费用
				spTO.setInsuranceDeputyFee(context.contextMap.get("INSURE_FEE")+"");//保险费押金代收款
				spTO.setcTR(paylist.get("TR_RATE")+"");//客户TR
				spTO.setaTR(paylist.get("TR_IRR_RATE")+"");//实际TR
				...
				for(int i=0;i<irrMonthPirces.length;i++) {
					Map<Enum<KeyOfMap>,Object> param=new HashMap<Enum<KeyOfMap>,Object>();
					param.put(SupportTO.KeyOfMap.IRR_MONTH_PRICE,irrMonthPirces[i]);
					param.put(SupportTO.KeyOfMap.START_PERIOD,startNums[i]);
					param.put(SupportTO.KeyOfMap.END_PERIOD,endNums[i]);
					spTO.getPayList().add(param);
				}
				List<Map<String,Object>> paylines=(List<Map<String,Object>>)paylist.get("paylines");
				double rateDiff=0;
				...
				for(int i=0;i<paylines.size();i++) {
					Map<Enum<KeyOfMap>,Object> param=new HashMap<Enum<KeyOfMap>,Object>();
					param.put(SupportTO.KeyOfMap.PERIOD_NUM,paylines.get(i).get("PERIOD_NUM"));
					param.put(SupportTO.KeyOfMap.IRR_MONTH_PRICE,paylines.get(i).get("IRR_MONTH_PRICE"));
					param.put(SupportTO.KeyOfMap.MONTH_PRICE,paylines.get(i).get("MONTH_PRICE"));
					param.put(SupportTO.KeyOfMap.OWN_PRICE,paylines.get(i).get("OWN_PRICE"));
					param.put(SupportTO.KeyOfMap.REN_PRICE,paylines.get(i).get("REN_PRICE"));
					param.put(SupportTO.KeyOfMap.LAST_PRICE,paylines.get(i).get("LAST_PRICE"));
					param.put(SupportTO.KeyOfMap.PV_PRICE,paylines.get(i).get("PV_PRICE"));
					param.put(SupportTO.KeyOfMap.VALUE_ADDED_TAX,paylines.get(i).get("VALUE_ADDED_TAX"));
					param.put(SupportTO.KeyOfMap.VALUE_ADDED_TAX_TRUE,paylines.get(i).get("VALUE_ADDED_TAX_TRUE"));
					spTO.getCollectionList().add(param);
					rateDiff=rateDiff+Double.valueOf(paylines.get(i).get("PV_PRICE").toString());
				}
				spTO.setRateDiff(ft.format(rateDiff));
				
				SqlMapClient sqlMapper=DataAccessor.getSession();
				try {
					sqlMapper.startTransaction();
					//I
					sqlMapper.update("common.updateLeaseTotalVal",spTO);//更新合同总金额
					
					//II
					List<Map<String,Object>> equList=sqlMapper.queryForList("common.getEquList",spTO);
					for(int i=0;equList!=null&&i<equList.size();i++) {
						equList.get(i).put("creditId",spTO.getCreditId());
						sqlMapper.update("common.updateEquipmentDetail",equList.get(i));
						//更新设备列表,只是更新单价和含税单价,如果需要更新其他内容,不可能修改支付表,而是应该重新录入新案,所以这里不考虑修改厂牌型号等信息
					}
					
					//III
					sqlMapper.update("common.updateCollectionPlan",spTO);//更新测算方案
					
					//IV
					//更新管理费与其他不影响TR之费用
					sqlMapper.update("common.updateFee1",spTO);
					sqlMapper.update("common.updateFee2",spTO);
					sqlMapper.update("common.updateFee3",spTO);
					sqlMapper.update("common.updateFee4",spTO);
					sqlMapper.update("common.updateFee5",spTO);
					sqlMapper.update("common.updateFee6",spTO);
					
					//V
					sqlMapper.update("common.updateCollectionIRR",spTO);//作废之前的还款计划,STATUS更新为-1
					Map<String,Object> param=new HashMap<String,Object>();
					param.put("creditId",spTO.getCreditId());
					for(int i=0;spTO.getPayList()!=null&&i<spTO.getPayList().size();i++) {
						param.put("IRR_MONTH_PRICE",spTO.getPayList().get(i).get(KeyOfMap.IRR_MONTH_PRICE));
						param.put("START_PERIOD",spTO.getPayList().get(i).get(KeyOfMap.START_PERIOD));
						param.put("END_PERIOD",spTO.getPayList().get(i).get(KeyOfMap.END_PERIOD));
						sqlMapper.insert("common.insertCollectionIRR",param);
					}
					//VI
					sqlMapper.update("common.updateMotorFee",spTO);//更新小车相关收费费等费用
					
					//VII
					for(int i=0;spTO.getCollectionList()!=null&&i<spTO.getCollectionList().size();i++) {
						param.put("PERIOD_NUM",spTO.getCollectionList().get(i).get(KeyOfMap.PERIOD_NUM));
						param.put("IRR_MONTH_PRICE",spTO.getCollectionList().get(i).get(KeyOfMap.IRR_MONTH_PRICE));
						param.put("MONTH_PRICE",spTO.getCollectionList().get(i).get(KeyOfMap.MONTH_PRICE));
						param.put("OWN_PRICE",spTO.getCollectionList().get(i).get(KeyOfMap.OWN_PRICE));
						param.put("REN_PRICE",spTO.getCollectionList().get(i).get(KeyOfMap.REN_PRICE));
						param.put("LAST_PRICE",spTO.getCollectionList().get(i).get(KeyOfMap.LAST_PRICE));
						param.put("PV_PRICE",spTO.getCollectionList().get(i).get(KeyOfMap.PV_PRICE));
						param.put("VALUE_ADDED_TAX",spTO.getCollectionList().get(i).get(KeyOfMap.VALUE_ADDED_TAX)==null?0:spTO.getCollectionList().get(i).get(KeyOfMap.VALUE_ADDED_TAX));
						param.put("VALUE_ADDED_TAX_TRUE",spTO.getCollectionList().get(i).get(KeyOfMap.VALUE_ADDED_TAX_TRUE)==null?0:spTO.getCollectionList().get(i).get(KeyOfMap.VALUE_ADDED_TAX_TRUE));
						sqlMapper.update("common.updateCollectionDetail",param);//更新支付表明细
					}
...
//			}
	...
	public void saveSchemaToSession(Context context){
		...
		map.put("pay__money", context.contextMap.get("pay__money"));
		map.put("incomePay", context.contextMap.get("incomePay"));
		map.put("incomePayType", context.contextMap.get("incomePayType"));
		map.put("outPay", context.contextMap.get("outPay"));
		System.out.println(map.get("incomePayType"));
		//Add by Michael 2012 01/29 增加费用保存
		List feeSetList =null;
	...
	public void updateSchema(Context context){
		...
		if(errList.isEmpty()){
			Output.jspSendRedirect(context, "../servlet/defaultDispatcher?__action=creditReport.selectCreditForUpdate&credit_id="+context.contextMap.get("CREDIT_ID")+"&showFlag=0");
		} else {
			outputMap.put("errList",errList) ;
			Output.jspOutput(outputMap, context, "/error.jsp") ;
		}
			 ...
			 context.contextMap.put("data_type", "客户来源");
				...
				if (schema != null) {
					...
					if("2".equals(schema.get("TAX_PLAN_CODE"))){
						List<Map> listTotalFeeSet=(List) DataAccessor.query("creditReportManage.getTotalFeeByRectID",context.contextMap, DataAccessor.RS_TYPE.LIST);
						for(Map map:listTotalFeeSet){
							totalFeeSet+=Math.ceil((DataUtil.doubleUtil(map.get("FEE"))/1.06));
						}	
					}else if("1".equals(schema.get("TAX_PLAN_CODE"))||"3".equals(schema.get("TAX_PLAN_CODE"))){
						totalFeeSet=(Double)DataAccessor.query("creditReportManage.sumTotalFeeByRectID",context.contextMap, DataAccessor.RS_TYPE.OBJECT);
					}
	...
	public void exportPaylist(Context context) {
		...
		try {
			...
			creditMap = (Map) DataAccessor.query("creditReportManage.selectCreditBaseInfo", context.contextMap, DataAccessor.RS_TYPE.MAP);
			// 
			context.contextMap.put("PAY_TAX_PLAN_CODE", context.contextMap.get("TAX_PLAN_CODE"));
			//---------------从页面取值--------------
			schema = copySchema(schema, context.contextMap);
...
//		    float[] widthsStl = {0.05f,0.10f,0.10f,0.10f,0.10f,0.10f,0.10f,0.10f,0.07f,0.10f};			
			...
			if("3".equals(context.contextMap.get("TAX_PLAN_CODE"))) {
				t2.addCell(makeCell("增值税", PdfPCell.ALIGN_CENTER, FontColumn));
			} else {
...
//				double realownprice = lastprice - pledge_ave_price;
	...
	public void paylistDetail (Context context) {
		...
		if (errList.isEmpty()) {
			Output.jspOutput(outputMap, context, "/credit/showPaylineDetail.jsp");
		} else {
			Output.jspOutput(outputMap, context, "/error.jsp");
		}
...
//Add by Michael 2012 2/2 填充补齐 净本金余额 等栏位信息
	...
	//产业分析页面查询 add by ShenQi 2012-8-23
    public void getIndustrialAnalyse(Context context) throws SQLException {
		...
		context.contextMap.put("data_type","客户来源");
		try {
			creditMap=(Map<String,Object>)DataAccessor.query("creditReportManage.selectCreditBaseInfo",context.contextMap,DataAccessor.RS_TYPE.MAP);
			...
			logger.debug("产业分析页面查询出错!");
			context.errList.add("产业分析页面查询出错!请联系管理员");
			e.printStackTrace();
		}
...
//		if("up".equals(context.contextMap.get("word"))) {
//			//报告审批
...
//		}
    ...
    //保存产业分析页面add by ShenQi 2012-8-23
	public void saveIndustrialAnalyse(Context context) throws SQLException {
		//移到公司沿革中
...
*/		
		String MARKET_STATUS=(String)context.contextMap.get("MARKET_STATUS");//市场规模及成长状况TextArea
		/*String POLICY_EFFECT=(String)context.contextMap.get("POLICY_EFFECT");//法规政策和未来科技影响TextArea
		String INDUSTRY_CHARACTER=(String)context.contextMap.get("INDUSTRY_CHARACTER");//产业特征TextArea
		String VALUE_CHAIN=(String)context.contextMap.get("VALUE_CHAIN");//产销价值链TextArea
		String BALK_BULLY=(String)context.contextMap.get("BALK_BULLY");//进入障碍及替代品威胁TextArea
*/		String RISK_ANALYSE=(String)context.contextMap.get("RISK_ANALYSE");//风险点分析TextArea
...
/*			sqlMapper.delete("creditReportManage.deleteMainProduction",context.contextMap);
			...
			sqlMapper.delete("creditReportManage.deleteIndustryEnvironment",context.contextMap);
			//插入6个TextArea栏位数据
			context.contextMap.put("MARKET_STATUS",MARKET_STATUS==null?"":MARKET_STATUS);
			/*context.contextMap.put("POLICY_EFFECT",POLICY_EFFECT==null?"":POLICY_EFFECT);
			context.contextMap.put("INDUSTRY_CHARACTER",INDUSTRY_CHARACTER==null?"":INDUSTRY_CHARACTER);
			context.contextMap.put("VALUE_CHAIN",VALUE_CHAIN==null?"":VALUE_CHAIN);
			context.contextMap.put("BALK_BULLY",BALK_BULLY==null?"":BALK_BULLY);*/
			context.contextMap.put("POLICY_EFFECT","");
			context.contextMap.put("INDUSTRY_CHARACTER","");
			context.contextMap.put("VALUE_CHAIN","");
			context.contextMap.put("BALK_BULLY","");
			context.contextMap.put("RISK_ANALYSE",RISK_ANALYSE==null?"":RISK_ANALYSE);
			sqlMapper.insert("creditReportManage.insertIndustryEnvironment",context.contextMap);
			
			sqlMapper.commitTransaction();
		} catch(Exception e) {
			logger.debug("产业分析页面保存出错!");
			context.errList.add("产业分析页面保存出错!请联系管理员");
			e.printStackTrace();
			try {
				sqlMapper.endTransaction();
		...
		this.getIndustrialAnalyse(context);
	}
}
