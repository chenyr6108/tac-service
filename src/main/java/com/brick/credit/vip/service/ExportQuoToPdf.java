...
//				t=t+monthPaylines.size();
				...
				for(int x = 0;x<irrMonthPaylines.size();x++){
			 		if(t==39||(t-39)%38==0){
					    tT2.addCell(makeCellSetColspanNoBorderTOP("     第"+irrMonthPaylines.get(x).get("IRR_MONTH_PRICE_START")+"-"+irrMonthPaylines.get(x).get("IRR_MONTH_PRICE_END")+"期每月RMB【"+  updateMon((irrMonthPaylines.get(x).get("IRR_MONTH_PRICE")+"" ))  +"】元", PdfPCell.ALIGN_LEFT, FontDefault,2));
					    t++;
			 		}else{
					    tT2.addCell(makeCellSetColspanLRBorder("     第"+irrMonthPaylines.get(x).get("IRR_MONTH_PRICE_START")+"-"+irrMonthPaylines.get(x).get("IRR_MONTH_PRICE_END")+"期每月RMB【"+  updateMon((irrMonthPaylines.get(x).get("IRR_MONTH_PRICE")+"" ))  +"】元", PdfPCell.ALIGN_LEFT, FontDefault,2));
					    t++;
			 		}
				}
				*/
