...
public class CreditVouchManage extends AService{
	Log logger = LogFactory.getLog(CreditVouchManage.class);
	...
	public void getAllCieditVouch(Context context) {
		...
		if(errList.isEmpty()){
			Output.jspOutput(outputMap, context, "/credit_vip/creditVouchShow.jsp");
		} else {
	...
	public void getAllCieditVouchManage(Context context) {
		...
		outputMap.put("type", context.contextMap.get("type"));
		if(errList.isEmpty()){
			Output.jspOutput(outputMap, context, "/credit_vip/creditVouchShowManage.jsp");
		} else {
			outputMap.put("errList", errList) ;
			Output.jspOutput(outputMap, context, "/error.jsp") ;
		}
	...
	//	List<Map> unionGrantPlan = null;
		...
		//	outputMap.put("supplier", supplier);
			...
			Output.jspOutput(outputMap, context,"/credit_vip/createProductVouch.jsp"); 
		} else {
			// 跳转到错误页面
			outputMap.put("errList", errList);
			Output.jspOutput(outputMap, context, "/error.jsp");
...
//				}
		...
		if (errList.isEmpty()) {
			outputMap.put("product", product);
			outputMap.put("grantplan", grantplan);
			Output.jspSendRedirect(context,"defaultDispatcher?__action=creditVouchManageVip.getAllCieditVouchManage");
...
//			Output.jspSendRedirect(context,"defaultDispatcher?__action=custCredit.queryAllCustCredit");
			Output.jspOutput(outputMap, context,"/credit_vip/showlog.jsp"); 
		} else {
			// 跳转到错误页面
			outputMap.put("errList", errList);
			Output.jspOutput(outputMap, context, "/error.jsp");
...
//				DataAccessor.getSession().update("productCredit.updateUnionPriceByID02",context.contextMap);
		...
		if (errList.isEmpty()){
			...
			Output.jspOutput(outputMap, context, "/servlet/defaultDispatcher?__action=creditVouchManageVip.getProductCreditJsp&PRODUCT_ID="+id+"&TYPE="+type);
		} else {
			outputMap.put("errList", errList) ;
			Output.jspOutput(outputMap, context, "/error.jsp") ;
		}
...
//				DataAccessor.getSession().update("productCredit.delUnionPriceByID02",context.contextMap);
	...
	public void getAllCieditByVouch (Context context) {
		...
		if(errList.isEmpty()){
			Output.jspOutput(outputMap, context, "/credit_vip/creditpact.jsp");
		}else{
			outputMap.put("errList", errList);
			Output.jspOutput(outputMap, context, "/error.jsp");
		}
