package com.brick.chartDirector.cylinder;
...
public class BaseCylinderChart {
	...
	public static ChartResult drawChart(HttpServletRequest request, ChartInfo chartInfo) throws Exception {
		...
		//创建图标
		XYChart c = new XYChart(chartInfo.chartWidth, chartInfo.chartHeight, 0xeeeeff, 0x000000, 1);
...
//		legendBox.setAlignment(Chart.TopRight);
		...
		//xy轴的密度
		//c.yAxis().setTickDensity(chartInfo.yDensity);
		...
		for (ChartDataSet dataSet : chartInfo.getChartDataList()) {
			...
			layer.addDataSet(dataSet.getyData(), -1, dataSet.getTitle()).setDataSymbol(Chart.CircleShape);
